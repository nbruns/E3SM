...
10          continue
          ...
          do v=1,met_nvars
            ...
            ! Modulo keeps index inside bound of valid years in cycle.
            if (yr .lt. 1850) then 
              atm2lnd_vars%tindex(g,v,1) = mod(yr-1 + (1850-mystart), nyears_spinup) * 365 * nint(24./atm2lnd_vars%timeres(v))
            else if (yr .le. atm2lnd_vars%endyear_met_spinup) then
              atm2lnd_vars%tindex(g,v,1) = mod(yr-1850 + (1850-mystart), nyears_spinup) * 365 * nint(24./atm2lnd_vars%timeres(v))
            else
